...
class MyRemoteCopyFile:
    ...
    #------------------------------------------------------------
    def _server_dir_prep(self, exp_id, gpc, group, src_dir, code):
        cwd = os.getcwd()
        ...
        for f in html_files:
            if int(f.split('.')[0][1:])==group:
                self._force_copy(os.path.join(html_path, f), os.path.join(self.web_directory, 'index.html'))
        
    #------------------------------------------------------------
    def run_copyfile(self, exp_id, gpc, group, src_dir, code):
       
        try:
            self._server_dir_prep(exp_id, gpc, group, src_dir, code)
            logger.info(f' Group ({group}) html file is copied to {gpc} successfully!')               
            return True
